...
def validate_mapping_keys(
    ...
    if identity in active_containers:
        raise ConfigError(f"{description} contains a recursive YAML value")
...
def load_state(path: Path) -> list[tuple[str, ...]]:
    ...
    for path_value in paths:
        if (
            ...
            or not path_value
            or not all(isinstance(part, str) for part in path_value)
        ):
...
def back_up_initial_config(
    ...
    if backup_path.is_symlink() or (backup_path.exists() and not backup_path.is_file()):
        raise ConfigError(f"stateful backup is not a regular file: {backup_path}")
    if state_path.exists() or not config_path.exists():
        return
